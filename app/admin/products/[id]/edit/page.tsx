...
export default function EditProductPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // Preserve list filter when returning after save (category & q from products list URL)
  const returnCategory = searchParams.get('category') ?? ''
  const returnQ = searchParams.get('q') ?? ''
  const productsListHref = (() => {
    const p = new URLSearchParams()
    if (returnCategory) p.set('category', returnCategory)
    if (returnQ) p.set('q', returnQ)
    const qs = p.toString()
      ...
      try {
        ...
        if (productData.product) {
          ...
          setFormData({
            ...
            volumeMl: prod.volumeMl != null ? prod.volumeMl : '',
            weightKg: prod.weightKg != null ? Number(prod.weightKg) : '',
    ...
    <div>
      ...
      {product && (
        <div className="mt-8 bg-white shadow rounded-lg p-6">
          ...
          <p className="text-sm text-gray-500 mb-4">Add one review manually or add category-based sample positive reviews (bags 18–24, foutas 23–44, others 50–100).</p>
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              >
                {seedReviewsLoading ? 'Adding reviews…' : 'Add category-based sample reviews to this product'}
              </button>
              {seedReviewsMessage && (

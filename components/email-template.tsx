...
export function orderConfirmationEmailHtml(props: OrderConfirmationProps): string {
  ...
  const pickupBlock = pickupAtLabel
    ? `<p style="margin:16px 0 0;color:${TEXT}"><strong>Scheduled pickup:</strong> ${escapeHtml(pickupAtLabel)}</p>`
    : "";

  const bodyHtml = `
  <h2 style="margin:0 0 12px;font-size:22px;color:${TEXT}">Thank you for your order</h2>
  ...
  <table style="width:100%;border-collapse:collapse;margin-top:16px;font-size:14px">
    <thead><tr style="border-bottom:2px solid ${BORDER};text-align:left">
      <th style="padding:8px 0;color:${TEXT_MUTED}">Item</th><th style="padding:8px 0;color:${TEXT_MUTED}">Qty</th>
      <th style="padding:8px 0;color:${TEXT_MUTED}">Unit</th><th style="padding:8px 0;color:${TEXT_MUTED}">Subtotal</th>
...
export function paymentFailedEmailHtml(props: PaymentFailedProps): string {
  ...
  const bodyHtml = `
  <h2 style="margin:0 0 12px;font-size:22px;color:${TEXT}">Payment not received</h2>
...
export function pickupReminderEmailHtml(props: PickupReminderProps): string {
  ...
  const bodyHtml = `
  <h2 style="margin:0 0 12px;font-size:22px;color:${TEXT}">Pickup reminder</h2>
...
export function shippingNotificationEmailHtml(props: ShippingNotificationProps): string {
  ...
  const bodyHtml = `
  <h2 style="margin:0 0 12px;font-size:22px;color:${TEXT}">Your order has shipped</h2>
  <p style="margin:0 0 8px;color:${TEXT_MUTED}">Order reference: <strong style="color:${TEXT}">${escapeHtml(orderNumber)}</strong></p>
  ${
    trackingNumber
...
export function OrderConfirmationEmail(props: OrderConfirmationProps) {
  const { orderNumber, items, total, subtotal, discountAmount, promoCode, shipping, pickupAtLabel, shippingAddress } = props;
  ...
  return (
    <html lang="en">
      ...
      <body style={{ fontFamily: "sans-serif", maxWidth: 560, margin: "0 auto", padding: 24, color: TEXT }}>
        ...
        {subtotal != null && (
          <p style={{ marginTop: 16, fontSize: 14, color: TEXT_MUTED }}>Subtotal: RM {Number(subtotal).toFixed(2)}</p>
        )}
        {discountAmount != null && discountAmount > 0 && (
          <p style={{ margin: "4px 0", fontSize: 14, color: SUCCESS }}>
            Promo{promoCode ? ` (${escapeHtml(promoCode)})` : ""}: -RM {Number(discountAmount).toFixed(2)}
          </p>
        )}
        {shipping != null && (
        ...
        {pickupAtLabel && (
          <p style={{ marginTop: 16 }}>
            <strong>Scheduled pickup:</strong> {escapeHtml(pickupAtLabel)}
          </p>
        )}
        {address && (address.fullName || address.address) && (
          <>
        ...
        <p style={{ marginTop: 24, color: TEXT_MUTED, fontSize: 14 }}>— Aquaheaven.com.my</p>
      </body>
    </html>
  );
}
  ...
  return (
    <html lang="en">
      ...
      <body style={{ fontFamily: "sans-serif", maxWidth: 560, margin: "0 auto", padding: 24, color: TEXT }}>
        ...
        {trackingUrl ? (
          <p style={{ marginTop: 16 }}>
            <a href={trackingUrl} style={{ color: BRAND, fontWeight: 600 }}>Track your parcel</a>
          </p>
        ) : null}
        {items && items.length > 0 ? (
          <>
  ...
  return (
    <html lang="en">
      ...
      <body style={{ fontFamily: "sans-serif", maxWidth: 560, margin: "0 auto", padding: 24, color: TEXT }}>
        ...
        <p style={{ marginTop: 16 }}>
          <a href={shopUrl} style={{ color: BRAND, fontWeight: 600 }}>Continue shopping</a>
          {" · "}
          <a href={supportUrl} style={{ color: BRAND, fontWeight: 600 }}>Customer support</a>
        </p>
        <p style={{ marginTop: 24, color: TEXT_MUTED, fontSize: 14 }}>— Aquaheaven.com.my</p>
      </body>
    </html>
  ...
  return (
    <html lang="en">
      ...
      <body style={{ fontFamily: "sans-serif", maxWidth: 560, margin: "0 auto", padding: 24, color: TEXT }}>
        ...
        <p style={{ marginTop: 16 }}>
          <a href={mapsUrl} style={{ color: BRAND, fontWeight: 600 }}>Open in Google Maps</a>
        </p>
        <p style={{ marginTop: 24, color: TEXT_MUTED, fontSize: 14 }}>— Aquaheaven.com.my</p>
      </body>
    </html>
